...
(function(global) {
  ...
  var HeteroscedasticRegressionLayer = function(opt) {
    ...
    // ok fine we will allow 'filters' as the word as well
    this.out_depth = typeof opt.num_neurons !== 'undefined' ? opt.num_neurons : opt.filters;
    global.assert(this.out_depth % 2 == 0, 'Error! output = [mean; sigma2] - num_neurons in last layer must be even.');
    ...
    this.out_sx = 1;
    this.out_sy = 1;
    this.layer_type = 'heteroscedastic_regression';
  ...
  HeteroscedasticRegressionLayer.prototype = {
    // The output of the network is of the structure [mean; sigma2]
    forward: function(V, is_training) {
      this.in_act = V;
      var A = new Vol(1, 1, this.out_depth, 0.0);
      var Vw = V.w;
    ...
    // y is a list here of size out_depth / 2
    backward: function(y) { 
      // compute and accumulate gradient wrt weights and bias of this layer
      var hLm1 = this.in_act;
      var hL = this.out_act;
      hLm1.dw = global.zeros(hLm1.w.length); // zero out the gradient of input Vol
      hL.dw = global.zeros(hL.w.length); // zero out the gradient of input Vol
      var loss = 0.0;
      // loop over D = out_depth / 2 dimensions
      for(var d=0;d<this.out_depth/2;d++) {
        var dy = hL.w[d] - y[d];
        var ls2 = hL.w[d + this.out_depth/2];
        var prec = Math.exp(-ls2)

        var Wmd = this.filters[d];
        var Wsd = this.filters[d + this.out_depth/2];
        for(var q=0;q<this.num_inputs;q++) {
          hLm1.dw[q] += prec*dy*Wmd.w[q]; // grad wrt input data
          hLm1.dw[q] -= 0.5*(prec*dy*dy - 1)*Wsd.w[q]; // grad wrt input data
          Wmd.dw[q] += prec*dy*hLm1.w[q]; // grad wrt params
          Wsd.dw[q] -= 0.5*(prec*dy*dy - 1)*hLm1.w[q]; // grad wrt params
        }
        this.biases.dw[d] += prec*dy;
        this.biases.dw[d + this.out_depth/2] -= 0.5*(prec*dy*dy - 1);
  ...
  global.HeteroscedasticRegressionLayer = HeteroscedasticRegressionLayer;
    ...
    makeLayers: function(defs) {
      ...
      for(var i=0;i<defs.length;i++) {
        ...
        switch(def.type) {
          ...
          case 'heteroscedastic_regression': this.layers.push(new global.HeteroscedasticRegressionLayer(def)); break;
      ...
      if(this.k % this.batch_size === 0) {
        ...
        for(var i=0;i<pglist.length;i++) {
          ...
          for(var j=0;j<plen;j++) {
            ...
            var gij = l2grad + l1grad + (g[j]) / this.batch_size; // raw batch gradient; Yarin: changed to div over g only
