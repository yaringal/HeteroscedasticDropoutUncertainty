...
var cnnvis = (function(exports){
  ...
  var MultiGraph = function(legend, options) {
    ...
    this.legend = legend;
    this.styles = options.styles || ["red", "blue", "green", "black", "magenta", "cyan", "purple", "aqua", "olive", "lime", "navy"];
  ...
  MultiGraph.prototype = {
    ...
    drawSelf: function(canv) {
      ...
      // draw legend
      ctx.fillStyle = "white";
      ctx.fillRect(W-pad-70, H-12-(pad+10*this.styles.length), W-pad-90, H-96-pad);
      for(var k=0;k<this.numlines;k++) {
        ctx.fillStyle = this.styles[k % this.styles.length];
        ctx.fillText(this.legend[k], W-pad-60, H+15-(pad+20+k*16));
      }
      ctx.fillStyle = "black";
      ...
      if(this.shaded > 0) {
        ctx.fillStyle = "red";
        ctx.globalAlpha=0.2;
        ctx.fillRect(pad, pad, Math.log(this.shaded) / Math.log(this.step_horizon) * (W-2*pad), H-pad*2);
        ctx.fillStyle = "black";
        ctx.globalAlpha=1;
      }
    }
